...
class V2User: NSObject {
    ...
    var user:UserModel? {
        ...
        set {
            //保证给user赋值是在主线程进行的
            //原因是 有很多UI可能会监听这个属性，这个属性发生更改是，那些UI也会相应的修改自己，所以要在主线程操作
    ...
    //全局once字符串，用于用户各种操作，例如回贴 登录。这些操作都需要用once，而且这个ocne是全局统一的
    var once:String? {
        get {
            //取了之后就删掉，
    ...
    /**
     获取并更新通知数量
     - parameter rootNode: 有Notifications的几点
     */
    func getNotificationsCount(_ rootNode: JiNode)  {
        //这里本想放在 JIHTMLResponseSerialier 自动获取。
